...
const _findOrderLowerThan = (items, order) => {
    let output = null;
...
const _findOrderUpperThan = (items, order) => {
    let output = null;
...
export const aboveItem = (state, selectedElementId) => {
    ...
    if (selectedElement && selectedElement.style && selectedElement.style.order) {
        above = _findOrderLowerThan(siblings, selectedElement.style.order);
    }
...
export const belowItem = (state, selectedElementId) => {
    ...
    if (selectedElement && selectedElement.style && selectedElement.style.order) {
        below = _findOrderUpperThan(siblings, selectedElement.style.order);
    }
...
export const getByVoiceTag = (elements, voiceTag) => {

    return Object.keys(elements).reduce((output, key) => {
        const element = elements[key];

        if (element.data && element.data.voiceTag && element.data.voiceTag === voiceTag) {
            output = element;
        }
